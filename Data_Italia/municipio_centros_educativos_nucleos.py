...
with open("C:/Users/vehico/Documents/Thesis/Distance-project/Data_Italia/school_by_municipality_complete.json", "r", encoding="utf-8") as f:
    school_data = json.load(f)

# Caricare lo shapefile dei comuni (estraiamo PRO_COM e nome comune)
comuni_gdf = gpd.read_file("comuni.zip")[["PRO_COM", "COMUNE"]]

# Caricare lo shapefile dei nuclei urbani (estraiamo PRO_COM e LOC21_ID)
nuclei_gdf = gpd.read_file("geometrias_Lazio.shp")[["PRO_COM", "LOC21_ID"]]
...
for comune, dati in school_data.items():
    ...
    if pro_com:
        ...
        school_data[comune]["NUCLEOS"] = nuclei_urbani  # Aggiunta della chiave "NUCLEOS"

# Salvare il file aggiornato
with open("school_by_municipality_with_nuclei.json", "w", encoding="utf-8") as f:
    json.dump(school_data, f, indent=4, ensure_ascii=False)

print("✅ File aggiornato con i nuclei urbani salvato come 'school_by_municipality_with_nuclei.json'")
