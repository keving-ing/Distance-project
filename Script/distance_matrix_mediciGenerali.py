...
def geocode_address(address):
    ...
    if data["status"] == "OK":
        ...
        log_message = f"❌ Geocoding error for '{address}': {data.get('status')}\n"
        print(log_message)
...
def get_distance_matrix(origins, destinations):
    ...
    params = {
        ...
        "key": GOOGLE_MAPS_API_KEY,
        "mode": "driving",
        "departure_time": 1746608400  # Consider current traffic
    }
    ...
    if data["status"] == "OK":
        ...
        log_message = f"❌ Distance Matrix API error for origins={origins} and destinations={destinations}: {data.get('status')}\n"
        print(log_message)
...
print("✅ Distances calculated and file saved! - elements:", elementi, " geocoding elements:", elementi_geo)
